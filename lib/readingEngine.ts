...
const cardNarratives: Record<number, CardNarrative> = {
  1: {
...
function getPairRelationship(firstCard: TarotCard, secondCard: TarotCard) {
  ...
  const hasTransformative =
    firstInfo.movement === "transformative" ||
    secondInfo.movement === "transformative";

  const hasHealing =
    firstInfo.movement === "healing" || secondInfo.movement === "healing";

  const hasActive =
    firstInfo.movement === "active" || secondInfo.movement === "active";

  const bothActive =
    firstInfo.movement === "active" && secondInfo.movement === "active";

  const hasReflective =
    firstInfo.movement === "reflective" ||
    secondInfo.movement === "reflective";

  const hasPassive =
    firstInfo.movement === "passive" || secondInfo.movement === "passive";

  const hasBalancing =
    firstInfo.movement === "balancing" || secondInfo.movement === "balancing";

  const hasCompleting =
    firstInfo.movement === "completing" || secondInfo.movement === "completing";

  const hasChallenging =
    firstInfo.energy === "challenging" || secondInfo.energy === "challenging";
  ...
  if (bothActive) {
    return {
      title: "harekete geçen güçlü irade",
      detail:
        "Bu ikili, beklemekten çok adım atma, kendini gösterme ve süreci hareketlendirme enerjisini anlatır.",
      advice:
        "Aksiyon al ama dağılma; enerjini tek bir net yöne bağla.",
    };
  }
  ...
  if (hasReflective && hasActive) {
    return {
      ...
      detail:
        "Bu ikili, önce içsel netlik arandığını; ardından bu farkındalığın hareket ve karar enerjisine dönüşebileceğini gösterir.",
      advice:
  ...
  if (hasPassive && hasActive) {
    return {
      ...
      detail:
        "Bu ikili, bir tarafın ilerlemek istediğini; diğer tarafın ise henüz bekleme, düşünme veya farklı açıdan bakma gerektirdiğini anlatır.",
      advice:
        "Hemen sonuç almaya çalışmak yerine doğru zamanlamayı ve bakış açını netleştir.",
    };
  }
  ...
  if (hasBalancing) {
    return {
      title: "denge kurarak ilerleme",
      detail:
        "Bu ikili, hızlı bir sonuçtan çok sabır, ölçü ve dengeli kararlarla ilerlemenin daha doğru olduğunu anlatır.",
      advice:
        "Aşırılardan uzak dur; sakin ve sürdürülebilir olanı seç.",
    };
  }

  if (hasReflective) {
    return {
      title: "içsel farkındalıkla yön bulma",
      detail:
        "Bu ikili, dışarıdaki gelişmeler kadar kişinin kendi içinde neyi fark ettiğinin de belirleyici olduğunu gösterir.",
      advice:
        "Cevabı sadece dışarıda arama; sende neyin değiştiğini de fark et.",
    };
  }

  return {
    title: "iki temanın aynı hikâyede birleşmesi",
    detail:
      "Bu ikili, iki farklı enerjinin aynı konuyu farklı açılardan desteklediğini gösterir.",
    advice:
